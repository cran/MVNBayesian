MixMVN_MCMC <-
function(data, steps, pars, values, tol=0.3, random_method = "Fast", ...){
  d <- length(data[1,2][[1]])
  if(any(pars %in% (d+1))){
    stop("Error assignment: conditioned parameters shouldn't include clusters")
  }
  else{
    MCMC <- MixMVN_GibbsSampler(steps, data, random_method = random_method, ...)
    samples_num <- length(MCMC[,1])
    if(length(pars) != length(values) | length(pars) > length(data[1,2][[1]])){
